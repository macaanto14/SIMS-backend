-- =====================================================
-- SIMS AUDIT TRAIL SYSTEM - FINAL VERSION
-- =====================================================
...
-- 
-- Author: Assistant
-- Version: 1.3.0 (Handles existing triggers)
-- Date: 2025-07-26
-- =====================================================
...
-- =====================================================
-- 6. AUDIT TRIGGER FUNCTION (CORRECTED)
-- =====================================================

-- Main trigger function for audit logging with corrected record field access
...
-- =====================================================
-- 9. DROP EXISTING TRIGGERS AND CREATE NEW ONES
-- =====================================================

-- Function to safely drop and recreate audit triggers for a table
CREATE OR REPLACE FUNCTION recreate_audit_trigger(table_name text)
RETURNS void AS $$
BEGIN
    ...
    RAISE NOTICE 'Recreated audit trigger for table: %', table_name;
END;
$$ LANGUAGE plpgsql;

-- Recreate audit triggers for main tables
SELECT recreate_audit_trigger('users');
SELECT recreate_audit_trigger('schools');
SELECT recreate_audit_trigger('student_profiles');
SELECT recreate_audit_trigger('teacher_profiles');
SELECT recreate_audit_trigger('parent_profiles');
SELECT recreate_audit_trigger('classes');
SELECT recreate_audit_trigger('attendance');
SELECT recreate_audit_trigger('grades');
SELECT recreate_audit_trigger('fee_payments');
SELECT recreate_audit_trigger('expenses');
SELECT recreate_audit_trigger('library_issues');
SELECT recreate_audit_trigger('user_roles');
SELECT recreate_audit_trigger('announcements');
SELECT recreate_audit_trigger('notifications');
