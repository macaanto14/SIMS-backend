...
if (Test-Path "auditService.ts") {
    Remove-Item "auditService.ts" -Force
    Write-Host "Removed lowercase auditService.ts"
}

# Create the corrected AuditService.ts file
$auditServiceContent = @"
...
# Write the corrected AuditService.ts file
$auditServiceContent | Out-File -FilePath "AuditService.ts" -Encoding UTF8 -Force
Write-Host "Created corrected AuditService.ts"

# Navigate back to project root
Set-Location "C:\Dugsinet\SIMS-backend"

Write-Host "Fixed AuditService case sensitivity and entity mapping issues"