@echo off
echo Testing SMS Verifications Table Fix
echo ===================================
...
echo.
echo.
echo Step 3: Testing with different Ethiopian format...
curl -X POST http://localhost:3000/api/sms/send-verification ^
  -H "Content-Type: application/json" ^
  -d "{\"phoneNumber\": \"0927802065\", \"purpose\": \"login\"}"

echo.
echo.
echo Step 4: Testing duplicate request (should update existing)...
curl -X POST http://localhost:3000/api/sms/send-verification ^
  -H "Content-Type: application/json" ^
  -d "{\"phoneNumber\": \"251927802065\", \"purpose\": \"registration\"}"
...
echo.
echo.
echo Fix and testing completed!
pause